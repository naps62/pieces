...
var _chunkX7BPFWKXcjs = require('./chunk-X7BPFWKX.cjs');
...
function createJobRunner(config) {
  ...
  async function processJob(pgBossJob) {
    ...
    await db.update(_chunkX7BPFWKXcjs.llmJobs).set({ status: "running", startedAt: /* @__PURE__ */ new Date() }).where(_drizzleorm.eq.call(void 0, _chunkX7BPFWKXcjs.llmJobs.id, jobId));
    let buffer = "";
    ...
    const flush = async () => {
      ...
      buffer = "";
      lastFlush = Date.now();
      await db.update(_chunkX7BPFWKXcjs.llmJobs).set({ log: _drizzleorm.sql`${_chunkX7BPFWKXcjs.llmJobs.log} || ${pending}` }).where(_drizzleorm.eq.call(void 0, _chunkX7BPFWKXcjs.llmJobs.id, jobId));
    };
    await executeSshJob(config.ssh, prompt, {
      ...
      onDone: async (exitCode, error) => {
        await flush();
        await db.update(_chunkX7BPFWKXcjs.llmJobs).set({
          status: exitCode === 0 ? "success" : "failed",
          exitCode: _nullishCoalesce(exitCode, () => ( null)),
          error: _nullishCoalesce(error, () => ( null)),
          finishedAt: /* @__PURE__ */ new Date()
        }).where(_drizzleorm.eq.call(void 0, _chunkX7BPFWKXcjs.llmJobs.id, jobId));
      }
    });
  }
  return {
    ...
    async enqueue(options) {
      const [row] = await db.insert(_chunkX7BPFWKXcjs.llmJobs).values({
        name: options.name,
        prompt: options.prompt,
        status: "pending"
      }).returning({ id: _chunkX7BPFWKXcjs.llmJobs.id });
    ...
    async getJob(id) {
      const rows = await db.select().from(_chunkX7BPFWKXcjs.llmJobs).where(_drizzleorm.eq.call(void 0, _chunkX7BPFWKXcjs.llmJobs.id, id)).limit(1);
      return _nullishCoalesce(rows[0], () => ( null));
    },
    async listJobs(options) {
      return db.select().from(_chunkX7BPFWKXcjs.llmJobs).orderBy(_drizzleorm.desc.call(void 0, _chunkX7BPFWKXcjs.llmJobs.createdAt)).limit(_nullishCoalesce(_optionalChain([options, 'optionalAccess', _ => _.limit]), () => ( 50)));
    }
  };
}
...
exports.createJobRunner = createJobRunner; exports.llmJobs = _chunkX7BPFWKXcjs.llmJobs;
